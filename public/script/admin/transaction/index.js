...
function displayData(type = 'all-transaction', id = null) {
    $(`#table-${type}`).DataTable();

    $("#state_transaction").val(type)

    if (data == "reschedule-reguler") {
        $.ajax({
            url: `/admin/transaction/reguler/reschedule-reguler/create`,
            method: "get",
            success: async function (response) {
                await $("#show-data-transaction").html(response);
                $(`#row-schedule-${id}`).addClass("success-alert");
                setTimeout(() => {
                    $(`#row-schedule-${id}`).removeClass("success-alert");
                }, 3000);
                await $(`#table-data-transaction`).DataTable();
            },
            error: function (err) {
                console.log(err);
            }
...
function openModal(data, type, id = null) {
    ...
    switch (type) {
        case "add":
            ...
            $(`#edit-${data}`).hide();
            $.ajax({
                url: `/admin/transaction/reguler/data-transaction-reguler/code`,
                method: "get",
                dataType: "json",
                success: async function (response) {
                    var code = `${data}`;
        ...
        case "edit":
            ...
            $(`#add-${data}`).hide();
            $.ajax({
                url: `/admin/transaction/reguler/data-transaction-reguler/${id}/edit`,
                method: "get",
                dataType: "json",
                success: async function (response) {
                    $(`#modal-${data}`).LoadingOverlay("hide");
        ...
        case "reschedule":
            ...
            $("#reschedule-date").val('')
            $.ajax({
                url: `/admin/transaction/reguler/reschedule-reguler/${id}/edit`,
                method: "get",
                success: function (response) {
                    // $("#seat-selection").html('');
                    $("#seat-selection").hide();
                    ...
                    $("#reschedule-booking_id").val(id)
                    $("#reschedule-booking_code").val(response.data.booking_transactions_code)
                    $("#reschedule-schedule_id").val(response.data.booking_transactions_schedule_id)
        ...
        case "payment":
            $(`#modal-payment-${data}`).LoadingOverlay("hide");
            $.ajax({
                url: `/admin/transaction/reguler/data-transaction-reguler/${id}`,
                method: "get",
                success: function (response) {
                    $("#payment-data-transaction").html(response)
                },
                error: function (err) {
                    console.log(err);
                }
    ...
    switch (type) {
        case "detail":
            $(`#form-${data}`).submit(function (e) {
                ...
                $.ajax({
                    url: `/admin/transaction/reguler/data-transaction-reguler`,
                    type: "post",
                    data: formData,
                    dataType: "json",
                    cache: false,
        ...
        case "reschedule":
            ...
            var formData = $("#reschedule-form").serialize();
            Swal.fire({
                title: "Yakin akan melakukan Reschedule?",
                text: "Reschedule hanya dapat dilakukan 1x selama transaksi!",
                icon: "warning",
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `/admin/transaction/reguler/reschedule-reguler/re-schedule`,
                        type: "post",
                        data: formData,
                        success: async function (response) {
                            console.log(response);
                            $("#modal-reschedule-reguler-transaction").modal("hide")
        ...
        case "delete":
            Swal.fire({
                ...
                text: "Data yang di hapus akan tersimpan pada Archieve",
                icon: "warning",
                showCancelButton: true,
                confirmButtonColor: "#3085d6",
                cancelButtonColor: "#d33",
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            var state_transaction = $("#state_transaction").val()
                            Toast.fire({
                                icon: "success",
                                title: message
                            });
...
function archieveData() {
    $("#modal-archieve-reguler-transaction").modal('show');
...
function printPDF(type) {
    var booking_code = $("#booking-transaction-code").val();
    if (type == 'purchase-order') {
        window.open(`/admin/transaction/reguler/data-transaction-reguler/print-invoice/${booking_code}`, "_blank");
    } else {
        window.open(`/admin/transaction/reguler/data-transaction-reguler/print-kwitansi/${booking_code}`, "_blank");
...
function submitPayment() {
    var id_booking_transaction = $("#id_booking_transaction").val();
    var nominal = $("#nominal_payment").val();
    var type_payment = $("#payment_method").val();
    var attachment = $("#attachment_payment").val();
    var outstanding_nominal = $("#outstanding-payment-data").val();
    console.log(nominal)
    console.log(outstanding_nominal)
    if (parseInt(nominal) > parseInt(outstanding_nominal)) {
        Toast.fire({
            icon: "error",
            title: "Nominal pengajuan tidak sesuai dengan nilai Outstanding Amount"
        });
        return;
    }
    ...
    $.ajax({
        type: "post",
        url: "/admin/transaction/reguler/data-transaction-reguler/submit-payment",
        data: {
            id_booking_transaction: id_booking_transaction,
            amount: nominal,
        ...
        success: function (response) {
            ...
            $(`#modal-payment-reguler-transaction`).modal('hide');
        }
    });
}