                ...
                <div class="card-content pt-1">
                    <div class="card-body">
                        <div class="table-responsive">
                            <input type="hidden" id="tablesPembayaran" value="{{$data->count()}}">
                            <table class="table datatables table-bordered table-hover"
                                   id="table-pembayaran-menu-keuangan">
                                <thead>
                                <tr>
                                    <th class="w-3p">No</th>
                                ...
                                @forelse($data as $item)
                                    @php
                                        $totalNominal += $item->nominal_pengajuan;
                                    @endphp
                                    <tr>
                                        <td>{{$no++}}</td>
                                        <td>{{$item->bank_name}}</td>
                                        <td>{{$item->pengajuan_no}}</td>
                                        <td>{{$item->tanggal_pengajuan}}</td>
                                        <td>{{'Rp'.number_format($item->nominal_pengajuan)}}</td>
                                        <td>{{$item->pic_pembayaran}}</td>
                                        <td>{{$item->description_pembayaran}}</td>
                                        <td>
                                            <div class="d-flex">
                                                <div
                                                    class="badge-circle badge-circle-sm badge-circle-primary mr-1 pointer"
                                                    data-toggle="modal"
                                                    data-target="#modal-detail-pembayaran{{$item->id}}">
                                                    <i class="bx bx-info-circle font-size-base"></i>
                                                </div>
                                                <div
                                                    class="badge-circle badge-circle-sm badge-circle-warning mr-1 pointer"
                                                    data-toggle="modal" title="edit"
                                                    data-target="#modal-edit-pembayaran{{$item->id}}">
                                                    <i class="bx bx-edit font-size-base"></i>
                                                </div>
                                                <div
                                                    class="badge-circle badge-circle-sm badge-circle-danger mr-1 pointer delete-pembayaran"
...
@push('page-scripts')
    <script>
        ...
        $(".delete-pembayaran").click(function (e) {
            e.preventDefault()
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '{{ route('finance-accounting-menu-keuangan-finance-pembayaran-delete') }}',
                        type: 'DELETE',
                        data: {
                            '_token': '{{ csrf_token() }}',
                            'id': pembayaranId
                        ...
                        success: function (response) {
                            ...
                            Toast.fire({
                                icon: "success",
                                title: "Berhasil menghapus data pembayaran"
                            });
                        },
                        error: function (err) {
                            console.log(err);
